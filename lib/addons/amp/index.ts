...
import {
  KubectlProvider,
  ManifestDeployment,
} from "../helm-addon/kubectl-provider";
...
export class AmpAddOn implements ClusterAddOn {
  ...
  deploy(clusterInfo: ClusterInfo): Promise<Construct> {
    ...
    if (
      typeof this.ampAddOnProps.prometheusRemoteWriteURL == "undefined" ||
      this.ampAddOnProps.prometheusRemoteWriteURL == null
    ) {
      cfnWorkspace = new aps.CfnWorkspace(
        cluster.stack,
        this.ampAddOnProps.workspaceName + "-amp-workspace",
        cfnWorkspaceProps
      ); /* all optional props */
      const ampUrlEndpoint = cfnWorkspace.attrPrometheusEndpoint;
      this.ampAddOnProps.prometheusRemoteWriteURL =
        ampUrlEndpoint + "api/v1/remote_write";
    }

    // Applying manifest for configuring ADOT Collector for Amp.
    if (this.ampAddOnProps.deploymentMode == DeploymentMode.DAEMONSET) {
      doc = readYamlDocument(
        __dirname + "/collector-config-amp-daemonset.ytpl"
      );
    } else {
    ...
    const values: Values = {
      ...
      // current region에 AMP resource가 없을 경우, 구성된 region 명시
      awsRegion: this.ampAddOnProps.region || cluster.stack.region,
